...
# eng_low = "abcdefghijklmopqrstuvwxyz"
# eng_up, eng_low = list(eng_low.upper()), list(eng_low)
# symbols = list("~!@#$%^&*()_+`-=№;:?/\|<>'{[}]")
...
#         curr_time = curr_time.strftime("%H%M%S") # 4-6 nums possible
#         # print(int(curr_time))
#         if (len(list(curr_time)) == 4):
#             pass
#         elif (len(list(curr_time)) == 5):
#             pass
#         elif (len(list(curr_time)) == 6):
#             pass
...
p = pypbkdf2.PyPBKDF2(salt_size=10) # 20
res = p.hash_password('Ксюша')
cipher_text = res[0]
salt = res[1]
# salt = "qwe123"
...
# p = pypbkdf2.PyPBKDF2(salt_size=100) # 20
# res = p.hash_password('')
# cipher_text = res[0]
# salt = res[1]

valid = p.verify_password('', cipher_text, salt)
